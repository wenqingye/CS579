...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    #compute each node, from level bottom up
    for n in node_distances:
        ...
        if n[0] != root:
            ...
            if length == 1:
                ...
                edge_value[b] = nv
            #nore than one parent, divide value
            else:
                ev = nv / length
                for m in parent:
                    node_value[m] += ev
...
def cut(S, T, graph):
    ...
    # e1, e2 for edges contain nodes from S and T
    for e in graph.edges():
        for n in range(0, len(S)):
            if e[0] == S[n] or e[1] == S[n]:
                e1.append(e)
    for e in graph.edges():
        for n in range(0, len(T)):
            if e[0] == T[n] or e[1] == T[n]:
                e2.append(e)

    # edges may have both ends in one set of nodes, exclude duplicates
    l1 = []
    l2 = []
    l1 = sorted(list(set(e1)))
    l2 = sorted(list(set(e2)))

    #find edges in both l1 and l2
...
def norm_cut(S, T, graph):
    ...
    vol_s = 1. * volume(S, graph)
    vol_t = 1. * volume(T, graph)
    cut_s_t = 1. * cut(S, T, graph)

    ncv = cut_s_t/vol_s + cut_s_t/vol_t
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
        result = partition_girvan_newman(graph, i)
        S = result[0].nodes()
        T = result[1].nodes()
...
def jaccard(graph, node, k):
    ...
    score_nodes = [x for x in graph.nodes() if x not in neighbor and x != node]   #exclude existing edges and self

    scores = []
    for n in score_nodes:
        neighbor2 = set(graph.neighbors(n))
        score = ((node, n), 1. * len(neighbor & neighbor2) / len(neighbor | neighbor2))
        scores.append(score)
...
def path_score(graph, root, k, beta):
    ...
    scores = []
    neighbor = graph.neighbors(root)
    score_nodes = [x for x in graph.nodes() if x not in neighbor and x != root]    #exclude existing edges and self
    for n in score_nodes:
        shortest_path_len, shortest_path_num, node2parents = bfs(graph, root, 5)     #self set max_depths
        shortest_path_len = shortest_path_len[n]
        shortest_path_num = shortest_path_num[n]
        sc = ((root, n), beta**shortest_path_len*shortest_path_num)
        scores.append(sc)
...
def evaluate(predicted_edges, graph):
    ...
    appear = [edge for edge in predicted_edges if graph.has_edge(*edge)]

    return 1. * len(appear) / len(predicted_edges)
